...
def resolve_model_path() -> Path:
    ...
    # Mulai dari parent directory (root project) karena main.py ada di machine-learning/
    root_dir = Path(__file__).parent.parent
    
    stable_path = root_dir / "runs/classify/train/weights/best.pt"
    if stable_path.exists():
        return stable_path

    candidates = sorted(
        [
            p for p in (root_dir / "runs").rglob("weights/best.pt")
    ...
    raise FileNotFoundError(
        f"Model best.pt belum ada di {stable_path}. Jalankan `python train.py` dulu."
    )
